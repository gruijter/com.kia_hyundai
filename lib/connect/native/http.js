...
const { RequestTimeoutError, NetworkError } = require('./exceptions');
const { normalizeLogger, redactBodyString } = require('./logger');

const USER_AGENT_OK_HTTP = 'okhttp/3.12.0';
const REQUEST_TIMEOUT_MS = 30 * 1000;

// Node's fetch reports connect/DNS failures as a generic "TypeError: fetch
// failed" with the real reason on error.cause (undici) — these codes mean the
// host couldn't be reached at all (router/firewall/ISP blocking outbound
// :8080, DNS failure, or the server being overloaded), not a credential or
// API problem. See issue #25.
const NETWORK_ERROR_CODES = new Set([
  'UND_ERR_CONNECT_TIMEOUT', 'UND_ERR_SOCKET', 'ETIMEDOUT', 'ECONNREFUSED', 'ECONNRESET',
...
function httpsRequestWithAgent(agent, method, url, { headers = {}, body, timeoutMs = REQUEST_TIMEOUT_MS } = {}) {
  return new Promise((resolve, reject) => {
    ...
    // destroy() above surfaces the RequestTimeoutError through 'error', so
...
class ApiImplSession {
  ...
  // Flat { name: value } view of the jar, for the login flows that hand a
  // throwaway session's cookies to a later call (see each region's
  // _getCookies).
  cookieMap() {
    return Object.fromEntries(this.cookies);
  }

  // Deliberate deviation from upstream, do NOT "fix" it: Python's
  // requests.Session scopes cookies per domain, this keeps ONE jar for every
  // host the session talks to. KiaUvoApiCN#login depends on that — its
  // five-step flow spans two hosts (prd.cn-ccapi + the UARS service) and
  // carries the session cookies across them. Scoping them per host is the
  // more correct behaviour in the abstract, but it would silently break that
  // login, and neither CN nor the cross-host EU steps can be validated
  ...
  // timeout: per-call override of REQUEST_TIMEOUT_MS, in ms. Only a few calls
  // need it — China's /location can take >30s while the car acquires a GPS
  // fix (upstream KiaUvoApiCN.py passes timeout=90 there). Ignored on the
  // httpsAgent path, which only the US regions use and which brings its own.
  async request(method, url, {
    headers = {}, body, redirect = 'follow', cookies, timeout,
  } = {}) {
    ...
    try {
      if (this.httpsAgent) {
        ...
        res = await fetch(url, {
          ...
          body,
          redirect,
          signal: AbortSignal.timeout(timeout || REQUEST_TIMEOUT_MS),
        });
      }
    } catch (error) {
      if (error.name === 'TimeoutError' || error.name === 'AbortError') {
        this.logger.error(`✗ ${logPath}: timed out after ${timeout || REQUEST_TIMEOUT_MS}ms`);
        throw new RequestTimeoutError(`${method} ${url} timed out`);
      }
      const code = (error.cause && error.cause.code) || error.code;
      if (code && NETWORK_ERROR_CODES.has(code)) {
        const { host } = new URL(url);
        this.logger.error(`✗ ${logPath}: cannot reach ${host} (${code})`);
        throw new NetworkError(`Cannot reach ${host} (${code})`);
      }
