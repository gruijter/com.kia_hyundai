...
const {
  BRAND_KIA, BRAND_HYUNDAI, REGION_AUSTRALIA, REGION_NZ,
} = require('../const');
...
class KiaUvoApiAU extends ApiImplType1 {
  constructor(region, brand, language, { logger } = {}) {
    super({ logger: prefixLogger(logger, 'KiaUvoApiAU') });
    ...
    if (brand === BRAND_KIA && region === REGION_AUSTRALIA) {
    ...
    } else if (brand === BRAND_KIA && region === REGION_NZ) {
      // Kia NZ — same host as Kia AU, its own service/app id. Reachable since
      // REGION_NZ was added to the dispatcher; before that this branch could
      // never be entered.
      this.BASE_URL = 'au-apigw.ccs.kia.com.au:8082';
      ...
      // Upstream leaves BASE_URL undefined here and fails later with an
      // opaque error; fail where the cause is still visible instead.
      throw new Error(`Unsupported brand/region for KiaUvoApiAU: ${brand} / ${region}`);
    }
  ...
  async updateVehicleWithCachedState(token, vehicleConfig) {
    ...
    const location = await this.getLocation(token, vehicleConfig);
    return KiaUvoApiAU._normalizeStatus(response.resMsg, location);
  }

  // AU/NZ's resMsg IS the flat status object (upstream wraps it as
  // {"status": resMsg} and then reads "status.time" / "status.odometer.value"),
  // so `time` is already in the right place for device.js#mapStatus(). The
  // odometer is not: mapStatus() reads it one level up, next to
  // vehicleStatus — the way EU's resMsg.vehicleStatusInfo already delivers
  // it — so hoist it here, or measure_odo never gets a value.
