...
// Poort van KiaUvoApiAU.py. Dekt Kia AU, Hyundai AU en Kia NZ (zoals upstream:
// dezelfde klasse, brand+region bepalen de eindpunten). Simpeler login-flow dan
// EU: geen RSA-encryptie, gewoon een JSON POST met e-mail/wachtwoord.
//
// Zelfde bewuste afwijking als KiaUvoApiEU.js: updateVehicleWithCachedState/
// forceRefreshVehicleState geven raw JSON terug i.p.v. een Vehicle-dataclass
// te muteren — genormaliseerd naar dezelfde { vehicleStatus, vehicleLocation,
// odometer } vorm als EU (upstream AU gebruikt zelf de key "status" i.p.v.
// "vehicleStatus"; hier omgezet zodat device.js#mapStatus() ongewijzigd werkt
// voor alle regio's). Zie ../../NAMING.md.
...
class KiaUvoApiAU extends ApiImplType1 {
  constructor(region, brand, language) {
    ...
    // gelijk aan Python's tuple(x * 0.5 for x in range(34, 54)) -> 17.0..26.5
    ...
    if (brand === BRAND_KIA && region === REGION_AUSTRALIA) {
      ...
      // Kia NZ (REGION_NZ) — enige andere combinatie die upstream ondersteunt
      this.BASE_URL = 'au-apigw.ccs.kia.com.au:8082';
  ...
  // Australië is altijd RHD, ongeacht de odometer-eenheid.
  _getDrvSeatLoc() {
    return 'R';
  }
  ...
  async _getAccessToken(authorizationCode, stamp) {
    ...
    // Refresh token blijft verbatim: de refresh_token-grant wijst een "Bearer "
    // prefix of access-token-in-plaats-van-refresh-token af (kia_uvo #1778).
    const refreshToken = response.refresh_token;
    const expiresIn = response.expires_in;
    return [tokenType, accessToken, refreshToken, expiresIn];
  }

  // AU vereist de Stamp-header op de refresh_token-grant.
