...
// Port of KiaUvoApiCN.py. CN deviates heavily from the ApiImplType1 base: no
// Stamp header, no ccs2/non-ccs2 distinction in the control actions (always
// the same endpoints), and a login flow with TWO sequential OAuth calls
// (authorization_code -> refresh_token grant using the just-received refresh
// token, which becomes the "refresh_token" in the Token). Because of this it
// overrides almost all ApiImplType1 methods instead of the EU/AU-style
// partial reuse.
//
...
class KiaUvoApiCN extends ApiImplType1 {
  ...
  // CN has no Stamp header, unlike the other Type1 regions.
  ...
  // CN doesn't distinguish between ccs2/non-ccs2 for the cached status.
  ...
  async _getAccessToken(authorizationCode) {
    ...
    // Upstream deliberately uses the refresh_token from this response here
    // as input for a SECOND oauth2/token call (_get_refresh_token) — not a
    // direct refresh_token grant like the other regions.
  ...
  async _getRefreshToken(authorizationCode) {
    ...
    const tokenType = response.token_type;
    // Upstream deliberately stores "<token_type> <access_token>" here as the
    // Token's refresh_token field (not a typo — see KiaUvoApiCN.py).
