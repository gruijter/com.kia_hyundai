...
// The bundle's ccspToken is the only part that must be present — without it
// there is no access token at all. extractUarsLoginBundle only guarantees
// data.uarsToken, so check here and fail readably instead of "Cannot read
// properties of undefined". Both spellings are accepted: the bundle is a
// server-rendered JS object and the pre-existing refresh_token fallback
// already conceded that its casing varies.
function readCcspToken(data, context) {
  const ccspToken = data?.ccspToken;
  const accessToken = ccspToken?.accessToken || ccspToken?.access_token;
  if (!accessToken) {
    throw new AuthenticationError(`${context}: UARS bundle carried no ccspToken.accessToken. `
      + 'The China login flow may have changed — check extractUarsLoginBundle first.');
  }
  return {
    accessToken,
    refreshToken: ccspToken.refresh_token || ccspToken.refreshToken,
    tokenType: ccspToken.tokenType || ccspToken.token_type || 'Bearer',
    expiresIn: Number(ccspToken.expiresIn ?? ccspToken.expires_in) || 21600,
  };
}
...
function extractUarsLoginBundle(html) {
  ...
  for (const keyMatch of html.matchAll(/"uarsToken"/g)) {
    ...
    // Walk BACKWARDS tracking brace depth, so only braces that actually
    // enclose the key are candidates — nesting depth, not every '{' in the
    // document. The previous version collected every preceding '{' and then
    // re-scanned to end-of-document from each: on a ~100KB page with a few
    // thousand braces that is ~10^8 comparisons, synchronously blocking
    // Homey's single-threaded sandbox mid-login.
    const openPositions = [];
    for (let i = keyAt, depth = 0; i >= 0; i -= 1) {
      if (html[i] === '}') depth += 1;
      else if (html[i] === '{') {
        if (depth === 0) openPositions.push(i);
        else depth -= 1;
      }
    }
    for (const start of openPositions) {
      let depth = 0;
...
class KiaUvoApiCN extends ApiImplType1 {
  ...
  constructor(region, brand, language, { logger } = {}) {
    ...
    // LOGIN_API_URL/CLIENT_ID have no reader left here since the UARS rewrite
    // replaced _getCookies — kept because upstream KiaUvoApiCN.py still sets
    // both (const lines 205/207), and dropping them would make the next
    // upstream diff harder to line up for no functional gain.
  ...
  async login(username, password, pin) {
    ...
    const ccspToken = readCcspToken(data, 'login');
    ...
    const { expiresIn } = ccspToken;
    return new Token({
      username,
      password,
      pin,
      accessToken: `${ccspToken.tokenType} ${ccspToken.accessToken}`,
      refreshToken: ccspToken.refreshToken,
      deviceId,
  ...
  async refreshAccessToken(token) {
    try {
      ...
      const { data } = await this._exchangeUarsCallback(redirectUrl);
      const ccspToken = readCcspToken(data, 'refreshAccessToken');
      const { expiresIn } = ccspToken;
