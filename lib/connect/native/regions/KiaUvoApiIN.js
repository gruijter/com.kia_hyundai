...
class KiaUvoApiIN extends ApiImplType1 {
  ...
  constructor(region, brand, language, { logger } = {}) {
    super({ logger: prefixLogger(logger, 'KiaUvoApiIN') });
  ...
  async updateVehicleWithCachedState(token, vehicleConfig) {
    ...
    return this._normalizeStatus(
      response.resMsg,
      await this.getLocation(token, vehicleConfig),
      await this._getMaintenanceAlert(token, vehicleConfig),
    );
  }
  ...
  // Port of _get_maintenance_alert: IN is the one Type1 region whose status
  // response carries no odometer at all — upstream reads it from this
  // separate endpoint in _update_vehicle_maintenance_alert (a bare number,
  // km per upstream's DISTANCE_UNITS[1]). Best-effort: a failure here must
  // not cost the whole status poll.
  async _getMaintenanceAlert(token, vehicleConfig) {
    try {
      const url = `${this.SPA_API_URL}vehicles/${vehicleConfig.id}/setting/alert/maintenance`;
      const response = await this.session.getJson(url, { headers: this._getAuthenticatedHeaders(token) });
      checkResponseForErrors(response);
  ...
  // IN's resMsg IS the flat status object (upstream's _update_vehicle_properties
  // reads "time"/"engine"/"airTemp.value" straight off it), so `time` already
  // sits where device.js#mapStatus() expects it. The odometer does not exist
  // there at all, so it comes from the maintenance-alert call above and is
  // placed next to vehicleStatus, the way EU's resMsg.vehicleStatusInfo
  // already delivers it — without that, measure_odo never gets a value.
  _normalizeStatus(status, location, maintenance) {
    const odometerValue = getChildValue(maintenance, 'odometer');
    return {
      vehicleStatus: status,
      vehicleLocation: location,
      // unit 1 = km, matching upstream's DISTANCE_UNITS[1] for this field.
      odometer: odometerValue === undefined ? undefined : { value: odometerValue, unit: 1 },
    };
  }
  ...
  async odometer(token, vehicleConfig) {
    ...
    return getChildValue(state, 'odometer') || { value: undefined };
  }
