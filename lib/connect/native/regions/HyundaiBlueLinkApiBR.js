...
class HyundaiBlueLinkApiBR extends ApiImplType1 {
  ...
  constructor(region, brand, language, { logger } = {}) {
    super({ logger: prefixLogger(logger, 'HyundaiBlueLinkApiBR') });
    ...
    // Upstream decorates exactly get_vehicles and update_vehicle_with_cached_state.
    // getVehicles is already wrapped by the base constructor (which wraps BR's
    // own override), so only this one is left to add; retryOnDeviceIdError is
    // idempotent now, but keep the list honest.
    applyRetryOnDeviceIdError(this, [
      'updateVehicleWithCachedState',
    ]);
  }
  ...
  async login(username, password, pin) {
    ...
    // `|| 86400` like every other region: a missing/invalid expires_in used to
    // yield an Invalid Date here, and VehicleManager._ensureValidToken's
    // `NaN < Date.now()` is false, so the token was never refreshed and every
    // command failed permanently once the server-side session really expired.
    const validUntil = new Date(Date.now() + (Number(auth.expires_in) || 86400) * 1000);
    ...
    if (token.refreshToken) {
      try {
        ...
        return new Token({
          ...
          validUntil: new Date(Date.now() + (Number(response.expires_in) || 86400) * 1000),
        });
      } catch {
  ...
  async forceRefreshVehicleState(token, vehicleConfig) {
    ...
    // Upstream raise_for_status()es both of these. Without the check an error
    // body leaves baselineTs undefined, and if the follow-up read also lacks
    // lastUpdateTime the two compare equal and this reports "did not return
    // fresh data" regardless of what actually went wrong.
    const pre = await this.session.getJson(latestUrl, { headers });
    checkResponseForErrors(pre);
    const baselineTs = pre.resMsg?.lastUpdateTime;

    const trigger = await this.session.getJson(triggerUrl, { headers });
    checkResponseForErrors(trigger);
    await wait(25 * 1000);
  ...
  // ApiImplType1#_getControlHeaders calls _getControlToken, but BR's own PIN
  // flow is _ensureControlToken (the name upstream uses too). Without this
  // alias every command BR does NOT override — startCharge, stopCharge,
  // setChargeLimits, setChargingCurrent, chargePortAction, setNavigation,
  // setVehicleToLoadDischargeLimit, scheduleChargingAndClimate — fell through
  // to the base implementation, which PUTs the EU-shaped `pin?token=` URL with
  // an Authorization header lacking the `Bearer ` prefix BR requires, so they
  // all failed as "PIN verification failed" even with the correct PIN.
  // Same class of deviation as the USER_API_URL/SPA_API_URL block above:
  // upstream raises on those paths, this makes them target the right flow.
  _getControlToken(token) {
    return this._ensureControlToken(token);
  }
