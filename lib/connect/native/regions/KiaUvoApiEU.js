...
// Poort van KiaUvoApiEU.py. Werkt voor zowel Kia als Hyundai EU (net als
// upstream, via het `brand`-argument) — Genesis is niet overgenomen omdat
// deze Homey-app geen Genesis-merk publiceert; poort de Genesis-tak uit
// upstream hier bij als dat ooit nodig is.
//
// Belangrijkste afwijking t.o.v. upstream: methodes die in Python een
// Vehicle-dataclass muteren (_update_vehicle_properties(_ccs2), ~1000 regels)
// zijn hier NIET overgenomen. In plaats daarvan geven updateVehicleWithCachedState
// en forceRefreshVehicleState de raw cloud-JSON terug, die device.js#mapStatus()
// al rechtstreeks kan lezen (dezelfde paden als de Python-parsing gebruikt).
// Zie ../../NAMING.md.
...
class KiaUvoApiEU extends ApiImplType1 {
  constructor(region, brand, language) {
    ...
    // gelijk aan Python's tuple(x * 0.5 for x in range(28, 60)) -> 14.0..29.5
  ...
  // LET OP (te verifiëren tijdens live EU-validatie): Node's fetch met
  // redirect:'follow' geeft alleen de headers van de UITEINDELIJKE response
  // terug, dus Set-Cookie headers op tussenliggende redirect-hops (stap 1
  // hieronder) worden niet opgepikt door de cookie-jar — Python's
  // requests.Session doet dat wel. Als login faalt op een ontbrekende
  // sessie-cookie, is dit de eerste plek om te fixen (self-follow met
  // redirect:'manual' + handmatige cookie-accumulatie per hop).
  async _loginWithPassword(username, password) {
    ...
    const s = new ApiImplSession();
    const baseHeaders = { 'User-Agent': mobileUa };

    // Stap 1: authorize-pagina laden voor sessie-cookies
    ...
    // Stap 2: RSA-publieke sleutel ophalen voor wachtwoordversleuteling
    ...
    // Stap 3: POST signin met versleuteld wachtwoord
    ...
    // Stap 4: authorization code inwisselen voor tokens
  ...
  async _forceRefreshVehicleStateCcs2(token, vehicleConfig) {
    // Wekt de auto, wacht (~25s live-gemeten) tot de auto rapporteert, en leest
    // dan de inmiddels verse cached snapshot — zie KiaUvoApiEU.py voor de reden
    // waarom de trigger-response zelf niet direct bruikbaar is.
  ...
  async _mergeCachedLocationPark(token, vehicleConfig, result) {
    try {
      ...
      // best-effort, net als upstream (_set_cached_location_park slikt fouten)
    }
  }
  ...
  // Poort van _get_odometer-equivalent gedrag: er is geen losse odometer-
  // endpoint in de EU-API, dus deze leest 'm uit de cached status.
  ...
  async _getCookies() {
    ...
    const session = new ApiImplSession();
