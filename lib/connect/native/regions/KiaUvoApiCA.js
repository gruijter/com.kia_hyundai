...
// Poort van KiaUvoApiCA.py. Andere backend dan EU/AU/CN (kiaconnect.ca /
// mybluelink.ca), met PIN-geverifieerde "pAuth"-tokens voor élk
// besturingscommando (niet gecached zoals EU's control-token) en een
// verplichte OTP-stap bij een onbekend apparaat (errorCode 7110).
//
// BEKENDE BEPERKING (nog te valideren, geen testaccount beschikbaar): de
// OTP-methodes (sendOtp/verifyOtpAndCompleteLogin) zijn wel geport, maar er is
// in deze Homey-app NOG GEEN pairing-UI-stap die ze aanroept — driver.js kan
// een OTP-uitdaging nu alleen als duidelijke fout tonen, niet interactief
// afhandelen. Zie ../../NAMING.md en het plan (Fase 3, CA) voor de vervolgstap.
//
// Zelfde bewuste afwijking als de andere regio's: raw JSON i.p.v. Vehicle-
// dataclass, genormaliseerd naar { vehicleStatus, vehicleLocation }.
...
// UUID5 (SHA-1, namespace DNS) — Node heeft geen ingebouwde uuidv5, Python's
// uuid.uuid5(NAMESPACE_DNS, ...) wel. Nodig voor een stabiel device_id
// (MAC+hostname), zodat herstarts niet telkens een nieuwe OTP triggeren.
...
class KiaUvoApiCA extends ApiImpl {
  constructor(region, brand, language) {
    ...
    // gelijk aan Python's temperature_range_c_old/_new
  ...
  // CA heeft geen refresh-endpoint; dit logt gewoon opnieuw in met het
  // bewaarde device_id, zodat de server het apparaat blijft herkennen.
  ...
  async startClimate(token, vehicleConfig, options = {}) {
    ...
    if (isEV) {
      ...
      // EV9 (Kia) / IONIQ 9 (Hyundai) gebruiken "remoteControl" i.p.v. "hvacInfo".
