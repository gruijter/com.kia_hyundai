...
// odometer }.
//
// Two CA-specific normalizations are needed to reach that shape, because
// this backend names things differently from the EU/AU convention that
// device.js#mapStatus() reads:
// - the status timestamp is `lastStatusDate` here, not `time` (upstream
//   reads "status.lastStatusDate"). mapStatus() gates its whole legacy
//   branch on `sts.time`, so without aliasing it NOTHING maps for Canada -
//   no lock state, battery, climate, range or alarms.
// - there is no odometer in the status at all: upstream takes it from the
//   separate `nxtsvc` (next service) call, as _getNextService does below.
...
const {
  APIError, AuthenticationError, AuthenticationOTPRequired, InvalidAPIResponseError, RateLimitingError,
} = require('../exceptions');
...
class KiaUvoApiCA extends ApiImpl {
  constructor(region, brand, language, { logger } = {}) {
    ...
    this._deviceId = null;
    // Port of the per-Vehicle state upstream's location gate reads
    // (vehicle.odometer / vehicle.location_latitude /
    // vehicle._location_fetch_attempted). This port returns raw JSON
    // instead of mutating a Vehicle dataclass, so the same two facts are
    // remembered here instead, keyed by vehicle id — same lifetime as
    // upstream's (in-memory, lost on restart), which is exactly what the
    // "fetch once after a restart" rule in #1294/#1296 is built around.
    this._locationState = new Map();
  }

  _locationStateFor(vehicleConfig) {
    const key = vehicleConfig.id;
    if (!this._locationState.has(key)) {
      this._locationState.set(key, {
        odometer: null, hasLocation: false, fetchAttempted: false, lastLocation: null,
      });
    }
    return this._locationState.get(key);
  }
  ...
  _checkResponseForErrors(response) {
    ...
    // Deviation from upstream: kiaconnect.ca/mybluelink.ca sit behind
    // Cloudflare, which answers a rate-limited client with its own 429 JSON
    // ({"status":429,"error_name":"rate_limited",...}) instead of the API's
    // responseHeader shape. Upstream then crashes on response['result'];
    // surface it as RateLimitingError so pairing and device.js can handle it.
    if (response && (response.status === 429 || response.error_name === 'rate_limited')) {
      throw new RateLimitingError(`Rate limited by ${this.BASE_URL} (${response.title || 'HTTP 429'})`);
    }
    if (!response || !response.responseHeader) {
      throw new InvalidAPIResponseError(`Unexpected response from ${this.BASE_URL}: ${(response && (response.title || response.detail)) || 'no responseHeader'}`);
    }
  ...
  // Port of _get_next_service: the `nxtsvc` call, which is where CA's
  // odometer comes from (the status response has none). Upstream reads
  // currentOdometer/currentOdometerUnit off it in
  // _update_vehicle_properties_service.
  async _getNextService(token, vehicleConfig) {
    const url = `${this.API_URL}nxtsvc`;
    const headers = { ...this.API_HEADERS, accessToken: token.accessToken, vehicleId: vehicleConfig.id };
    const response = await this.session.postJsonExpectJson(url, undefined, { headers });
    this._checkResponseForErrors(response);
    return response.result.maintenanceInfo;
  }

  // Port of the location gate in update_vehicle_with_cached_state /
  // force_refresh_vehicle_state (upstream #1294 + #1296): CA's location
  // comes only from the PIN-protected Find-My-Car call, so it is fetched
  // when the car has moved since the last service reading, or once per
  // session while no location is known yet. Without the gate every poll
  // spends a vrfypin + fndmcr pair (_getPinToken is not cached here, same
  // as upstream).
  async _locationIfNeeded(token, vehicleConfig, service) {
    const state = this._locationStateFor(vehicleConfig);
    const currentOdometer = getChildValue(service, 'currentOdometer');
    let location = null;
    if (state.odometer) {
      if (state.odometer < currentOdometer || (!state.hasLocation && !state.fetchAttempted)) {
        if (!state.hasLocation) state.fetchAttempted = true;
        location = await this.getLocation(token, vehicleConfig);
      }
    } else {
      location = await this.getLocation(token, vehicleConfig);
    }
    // Remember the odometer after the fact, so the comparison above still
    // sees the previous reading — same ordering (and same reason) as
    // upstream's "_update_vehicle_properties_service after the fact".
    state.odometer = currentOdometer;
    if (getChildValue(location, 'coord.lat') !== undefined) {
      state.hasLocation = true;
      state.lastLocation = location;
    }
    // Upstream leaves vehicle.location untouched when the gate doesn't
    // fire, i.e. the previously known position stays visible. Return that
    // same last-known value rather than null: device.js#doPoll() fetches
    // the location itself whenever vehicleLocation is missing on a
    // non-CCS2 car, which would re-add exactly the per-poll call this gate
    // exists to avoid.
    return location || state.lastLocation || null;
  }

  // Normalizes to the { vehicleStatus, vehicleLocation, odometer } shape
  // device.js#mapStatus() reads. `time` aliases CA's own `lastStatusDate`
  // (see the file header) and the odometer comes from the nxtsvc call.
  static _normalizeStatus(status, service, location) {
    return {
      vehicleStatus: { ...status, time: getChildValue(status, 'lastStatusDate') },
      vehicleLocation: location,
      odometer: {
        value: getChildValue(service, 'currentOdometer'),
        unit: getChildValue(service, 'currentOdometerUnit'),
      },
    };
  }
  ...
  async updateVehicleWithCachedState(token, vehicleConfig) {
    ...
    const service = await this._getNextService(token, vehicleConfig);
    const location = await this._locationIfNeeded(token, vehicleConfig, service);
    return KiaUvoApiCA._normalizeStatus(status, service, location);
  }
  ...
  async startClimate(token, vehicleConfig, options = {}) {
    ...
    const hexSetTemp = getIndexIntoHexTemp(nearestRangeIndex(range, setTemp));
