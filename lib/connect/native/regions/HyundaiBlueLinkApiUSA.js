...
// Poort van HyundaiBlueLinkApiUSA.py. Andere backend dan Kia USA
// (api.telematics.hyundaiusa.com, headers-gebaseerde auth met "accessToken" +
// "blueLinkServicePin"), maar de raw statusvorm (vehicleStatus.airCtrlOn,
// .doorLock, .doorOpen.frontLeft, .evStatus.*, ...) is vrijwel identiek aan de
// EU/oude-stijl conventie — hier dus vrijwel geen normalisatie nodig, in
// tegenstelling tot Kia USA. Zie ../../NAMING.md.
//
// Net als Kia USA vereist de server een verlaagd TLS-securityniveau
// (SECLEVEL=1, zie cipherAdapter in de Python-bron). ONGETEST: geen Hyundai
// USA-account beschikbaar om dit tegen te valideren.
...
// Controlecommando's geven vaak HTTP 200 met een lege body terug.
...
class HyundaiBlueLinkApiUSA extends ApiImpl {
  constructor(region, brand, language) {
    ...
    this.session = new ApiImplSession({ httpsAgent: HYUNDAI_USA_HTTPS_AGENT });
  ...
  async startClimate(token, vehicleConfig, options = {}) {
    ...
    if (isEV) {
      ...
      // Oudere generaties ondersteunen geen seat-heater-vent-info of duration.
