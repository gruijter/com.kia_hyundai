...
// Port of upstream #1264's max(...) over both odometer readings, using the
// same float_or_none coercion (a non-numeric reading drops out rather than
// becoming 0/NaN). Returns undefined when neither field carries a number.
function freshestOdometer(...values) {
  const numbers = values
    .map((value) => (value === null || value === undefined ? NaN : Number(value)))
    .filter((value) => Number.isFinite(value));
  return numbers.length ? { value: Math.max(...numbers) } : undefined;
}
...
class HyundaiBlueLinkApiUSA extends ApiImpl {
  ...
  // Normalizes to the { vehicleStatus, odometer } shape device.js#mapStatus()
  // reads, the same way KiaUvoApiUSA#_normalizeStatus does for the Kia USA
  // backend. Two renames are needed:
  // - `time`: this backend's status timestamp is `dateTime`. mapStatus()
  //   gates its whole legacy branch on `sts.time`, so without the alias
  //   NOTHING maps for Hyundai USA — no lock state, battery, climate,
  //   range or alarms.
  // - `odometer`: there is none in the status response; it comes from the
  //   enrollment details. Port of upstream #1264 — take the freshest of the
  //   two readings, because vehicleDetails comes from the enrollment cache
  //   (deliberately kept for the instance lifetime, as upstream does) while
  //   vehicleStatus.odometer, when present, is from this very call.
  async _buildStatus(token, vehicleConfig, refresh) {
    ...
    return {
      vehicleStatus: { ...vehicleStatus, time: vehicleStatus.dateTime },
      // Top level, next to vehicleStatus — NOT inside it, which is where this
      // used to put it. mapStatus() reads sts.vehicleLocation off the outer
      // object, so a nested one is invisible: no position, no speed, no
      // home-distance. It also left device.js#doPoll()'s "no location on a
      // non-CCS2 car" fallback firing every poll, costing a 5s wait and an
      // error, since this class has no public getLocation() for it to call.
      vehicleLocation: location || undefined,
      odometer: freshestOdometer(details?.odometer, vehicleStatus?.odometer),
    };
  }
  ...
  async odometer(token, vehicleConfig) {
    const details = await this._getVehicleDetails(token, vehicleConfig);
    return freshestOdometer(details?.odometer) || { value: undefined };
  }
