...
class ApiImpl {
  ...
  async scheduleChargingAndClimate(token, vehicleConfig, options) {
    this.notImplemented('scheduleChargingAndClimate');
  }

  // Deliberately deviates from upstream: upstream splits this into
  // update_month_trip_info/update_day_trip_info, each mutating a Vehicle
  // dataclass. This returns the raw /tripinfo response instead, matching
  // KiaUvoApiEU.js#drivingInfo — see ../NAMING.md.
  // eslint-disable-next-line no-unused-vars
  async tripInfo(token, vehicleConfig, dateString, period) {
    this.notImplemented('tripInfo');
  }
