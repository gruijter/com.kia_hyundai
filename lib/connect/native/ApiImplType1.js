...
const { normalizeLogger } = require('./logger');
const { VEHICLE_LOCK_ACTION, ORDER_STATUS } = require('./const');
const { getIndexIntoHexTemp, nearestRangeIndex, formatScheduleTime } = require('./utils');
...
// Port of ApiImplType1.py#error_message_to_exception_mapping: only these two
// `error` values mean the session actually needs re-authenticating. Any other
// `error` is an APIError upstream — mapping them all to AuthenticationError
// (as this did before) tells the user their password is wrong and parks the
// device on device.js's 15-minute auth retry for what is really a protocol
// or server-side problem (e.g. invalid_client, unsupported_grant_type).
const ERROR_MESSAGE_MAPPING = {
  ...
  // Deliberate addition beyond upstream's two strings: OAuth's invalid_grant
  // is exactly "this credential is dead, re-authenticate" (revoked refresh
  // token, changed password). Upstream lets it fall through to APIError
  // because Home Assistant does not branch on the type; device.js does — it
  // shows device_auth_failed for AuthenticationError and the generic
  // "restarting" message for anything else.
...
function checkResponseForErrors(response) {
  ...
  if (response.error) {
    const ErrorClass = ERROR_MESSAGE_MAPPING[response.error];
    if (ErrorClass) throw new ErrorClass(response.error);
    throw new APIError(`Unknown error in API response: ${response.error}`);
  }
...
function retryOnDeviceIdError(fn) {
  // Idempotent. Python applies @_retry_on_device_id_error at class-definition
  // time, so a subclass that re-decorates its own override still gets exactly
  // one wrapper. Here the base constructor wraps whatever `this[name]`
  // resolves to — which for a subclass instance is already the subclass's
  // override — so a subclass listing the same name again wrapped it twice:
  // one DeviceIDError then cost 4 attempts and 3 device-id registrations
  // instead of 2 and 1. BR, IN and CN all did that for at least one method.
  if (fn.retriesOnDeviceIdError) return fn;
  const wrapped = async function wrapped(token, ...args) {
    try {
  ...
  wrapped.retriesOnDeviceIdError = true;
  return wrapped;
}

// Wrap+bind a list of method names on `target`, skipping any already wrapped.
// The marker has to be re-applied after .bind(), which returns a fresh
// function object and does NOT carry custom properties across — without that
// the idempotence check above never sees it and the double-wrap comes back.
function applyRetryOnDeviceIdError(target, names) {
  names.forEach((name) => {
    if (typeof target[name] !== 'function' || target[name].retriesOnDeviceIdError) return;
    const bound = retryOnDeviceIdError(target[name]).bind(target);
    bound.retriesOnDeviceIdError = true;
    // eslint-disable-next-line no-param-reassign
    target[name] = bound;
  });
}
...
class ApiImplType1 extends ApiImpl {
  // logger is passed up from the subclass (already prefixed with the region
  // class name) so the session built here is the one that actually logs.
  // Upstream's ApiImplType1.__init__ builds the session the same way; it just
  // has no logger concept, which is a port-local addition throughout.
  constructor({ logger } = {}) {
    super();
  ...
  async startClimate(token, vehicleConfig, options = {}) {
    ...
    if (!vehicleConfig.ccuCCS2ProtocolSupport) {
      ...
      const hexSetTemp = getIndexIntoHexTemp(nearestRangeIndex(this.temperatureRange, setTemp));
      payload = {
        action: 'start',
        hvacType: 0,
      ...
      payload = {
        ...
        hvacTempType: 1,
        // Snapped to the region's own valid temperatureRange steps for the
        // same robustness reason as the non-CCS2 hexSetTemp lookup above -
        // upstream sends options.set_temp here completely unclamped/
        // unrounded, so a value that isn't already one of the range's exact
        // steps (e.g. Fahrenheit-UI-origin Celsius) would be forwarded as-is.
        hvacTemp: this.temperatureRange[nearestRangeIndex(this.temperatureRange, setTemp)],
  ...
  async scheduleChargingAndClimate(token, vehicleConfig, options = {}) {
    ...
    payload.offPeakPowerInfo = {
      ...
      // Flag semantics match the apps' own vehicle data model (upstream
      // #1304, which corrected an inversion on both sides): 1 = considering
      // peak time ("off-peak tariffs prioritised"), 2 = only peak time
      // ("off-peak tariffs only"), 0 = not applied. The read side lives in
      // device.js#buildScheduleOptions() and must stay in step with this.
      offPeakPowerFlag: offPeakChargeOnlyEnabled ? 2 : 1,
    };
  ...
  async _scheduleEv5Flat(token, vehicleConfig, options) {
    ...
    const chargePayload = {
      ...
      // Same enum as the combined path above: 2 = only off-peak, 1 = prioritised.
      offpeakPowerFlag: offPeakChargeOnlyEnabled ? 2 : 1,
...
module.exports = {
  ApiImplType1, checkResponseForErrors, retryOnDeviceIdError, applyRetryOnDeviceIdError,
};
