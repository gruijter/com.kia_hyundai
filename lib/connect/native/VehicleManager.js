...
const { REGION_EUROPE, VEHICLE_LOCK_ACTION } = require('./const');
const KiaUvoApiEU = require('./regions/KiaUvoApiEU');

const TOKEN_REFRESH_BUFFER_MS = 5 * 60 * 1000;

// Poort van VehicleManager.py: kiest de regio-implementatie, beheert het
// token (login + refresh-before-expiry) en delegeert commando's. Alleen EU
// is momenteel geïmplementeerd; andere regio's volgen in latere fases (zie
// het plan) en gooien hier bewust een duidelijke fout tot ze zijn geport.
class VehicleManager {
  ...
  static getImplementationByRegionBrand(region, brand, language) {
    ...
    throw new Error(`No native implementation for region "${region}" yet — falls back to bluelinky (see lib/connect/index.js)`);
  }
