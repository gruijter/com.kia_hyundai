...
// Port of the day-of-week list comprehension in ApiImplType1.py's CCS2
// reservation parser. Returns weekday indices in Date#getDay() convention
// (0 = Sunday) for the days a CCS2 Departure.Schedule{1,2} repeats on.
function ccs2DepartureDays(schedule) {
  if (!schedule) return [];
  return ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat']
    .map((key, index) => (schedule[key] === 1 ? index : -1))
    .filter((index) => index >= 0);
}

// Given a wall-clock { hours, minutes }, a set of weekday indices
// (Date#getDay() convention, 0 = Sunday; empty/none = every day) and an IANA
// timeZone, returns a Date whose UTC fields hold the local wall-clock value
// of the next future occurrence — so the caller formats it with
// timeZone: 'UTC'. Returns null if no time was given. Never builds a real
// instant: a weekly departure schedule is wall-clock and must not shift
// across a DST boundary.
function nextDepartureOccurrence(time, days, timeZone, now = new Date()) {
  if (!time) return null;
  ...
  const nowLocal = Date.UTC(parts.year, parts.month - 1, parts.day, parts.hour % 24, parts.minute, parts.second);
  let weekdays = (days || []).map(Number).filter((day) => day >= 0 && day <= 6);
  if (!weekdays.length) weekdays = [0, 1, 2, 3, 4, 5, 6]; // e.g. legacy [9] sentinel
  for (let offset = 0; offset <= 7; offset += 1) {
    const candidate = new Date(Date.UTC(parts.year, parts.month - 1, parts.day + offset, time.hours, time.minutes, 0));
    if (weekdays.includes(candidate.getUTCDay()) && candidate.getTime() > nowLocal) return candidate;
  }
  return null;
}
