...
function buildVehicleDebugDump({
  brand, region, engine, generation, ccuCCS2ProtocolSupport, vehicleConfig, status, odometer, drivingInfo,
}) {
  return {
    meta: {
      brand,
    ...
    // energy-consumption / driving-history stats (lib/connect/native/regions/
    // KiaUvoApiEU.js#drivingInfo) — a separate endpoint from `status`, only
    // fetched for EV/PHEV vehicles, so undefined (and dropped by
    // JSON.stringify) for HEV/ICE.
    drivingInfo: drivingInfo ? redactPII(redact(drivingInfo)) : undefined,
  };
}
