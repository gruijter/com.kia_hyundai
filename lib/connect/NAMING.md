...
|-------|--------|-------------------|
| EU (Kia + Hyundai) | **Live validated** (real Kia Niro HEV/PHEV + Niro EV, 2026-08-04). Password login was ported to the OneApp/CCI flow on 2026-08-17 (upstream #1273/#1277-#1279, WAF block on the old IDPConnect authorize endpoint) — re-validated only via the fake-credentials mock test against the real servers (reaches signin, no WAF block), **not yet with a real account/car**. | `KiaUvoApiEU.js#_loginWithPasswordCci` step 1 (authorize call) can miss Set-Cookie headers on intermediate redirect hops — Node's `fetch` with `redirect:'follow'` only returns the headers of the final response, Python's `requests.Session` accumulates across all hops. Hasn't proven to be a problem in the live test yet, but is the first checkpoint for a login issue. Steps 4/5 (CCI token exchange, CCS re-exchange) and `_refreshCciToken` are ported faithfully from upstream but **untested end-to-end** — no valid credentials were available to reach past the signin step. |
