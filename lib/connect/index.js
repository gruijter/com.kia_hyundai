...
const REGION_MAP = {
  ...
  NZ: constants.REGION_NZ,
  IN: constants.REGION_INDIA,
  BR: constants.REGION_BRAZIL,
};
const BRAND_MAP = { kia: constants.BRAND_KIA, hyundai: constants.BRAND_HYUNDAI };

// Which regions each brand actually has a backend for, in the order they are
// offered. The single source of truth for the pairing/repair region picker
// (drivers/car/driver.js answers a 'regions' request from it), so a brand can
// never be offered a region that would only fail at login. Two are one-sided:
// Brazil is Hyundai only and New Zealand is Kia only — neither region has a
// backend for the other brand.
const REGIONS_BY_BRAND = {
  kia: ['EU', 'US', 'CA', 'AU', 'NZ', 'CN', 'IN'],
  hyundai: ['EU', 'US', 'CA', 'AU', 'CN', 'IN', 'BR'],
};

// Homey-side region ids this app supports for `brand` ('kia' | 'hyundai').
function regionsForBrand(brand) {
  return REGIONS_BY_BRAND[brand] || REGIONS_BY_BRAND.kia;
}
...
module.exports = {
  createClient, regionsForBrand, exceptions, constants,
};
