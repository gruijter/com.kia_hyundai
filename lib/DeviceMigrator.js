...
module.exports = {
  ...
  // Returns whether it actually touched the list — a removed-and-re-added
  // capability comes back with the manifest's capability options, so any
  // caller caching what it last wrote with setCapabilityOptions() has to drop
  // that cache (see syncControlVisibility() and reconcileUnitMarkers()).
  ...
  // Hides or restores the two REMOTE-CONTROL capabilities in Homey's UI, per
  // the device's `disableDoorWindowControl` setting. `uiComponent: null` is
  // what removes a capability's control from the device page; the capability
  // itself stays in place, so flow cards (including Homey's own system cards
  // for `locked`) and every flow-card device filter keyed on
  // `capabilities=vent_windows` keep working. That filter is exactly why the
  // capabilities are NOT removed here — see driver.js#capabilitiesToCheck.
  //
  // Hiding the control is cosmetic on its own: the capability stays setable,
  // so device.js#lock()/#setWindows() also refuse a 'app'/'widget' source
  // while the setting is on. Both halves are needed.
  //
  // Same store-marker approach as sync*Units() below: setCapabilityOptions()
  // is documented as expensive, this runs on every onInit(), and reading
  // getCapabilityOptions() back to decide can throw right after a boot.
  async syncControlVisibility(device, hidden) {
    const caps = ['locked', 'vent_windows'];
    const target = hidden ? 'hidden' : 'visible';
    if (device.getStoreValue('appliedControlVisibility') === target) return;
    const manifestOptions = device.driver.manifest.capabilitiesOptions || {};
    let allApplied = true;
    for (const cap of caps) {
      if (!device.hasCapability(cap)) continue;
      device.log(`${target === 'hidden' ? 'hiding' : 'restoring'} ${cap} control for ${device.getName()}`);
      const baseOptions = manifestOptions[cap] || {};
      // Merged onto the manifest options rather than sent alone, so a
      // per-capability title/icon override isn't silently dropped. Restoring
      // means writing the manifest options back untouched, which is what the
      // capability had before this ever ran.
      const options = hidden ? { ...baseOptions, uiComponent: null } : { ...baseOptions };
      // eslint-disable-next-line no-await-in-loop
      const applied = await device.setCapabilityOptions(cap, options)
        .then(() => true)
        .catch((error) => {
          device.error(error);
          return false;
    ...
    if (allApplied) await device.setStoreValue('appliedControlVisibility', target).catch((error) => device.error(error));
  },

  // The appliedDistanceUnits/appliedSpeedUnits/appliedFuelEconomyUnits store
  // markers below are only a cache of what sync*Units() last wrote, and a
  // cache can go stale: a capability that gets removed and re-added
  // (migrateCapabilities() above, on any release that inserts a capability
  // ahead of the distance ones) comes back carrying the driver manifest's own
  // metric options, while the marker still claims mi/mph — after which the
  // sync early-returns forever and nothing ever restores the label. 2.4.0
  // shipped exactly that, by inserting departure_schedule.1/.2 ahead of
  // meter_distance/measure_speed/measure_range/measure_odo: affected cars kept
  // reporting correct mileage figures, labelled km (community report #1050).
  //
  // So rather than special-casing that one migration, check every marker
  // against the capability options actually in place and drop any that lies.
  // The next doPoll() then re-applies whatever unit the incoming car data
  // reports, so the device heals itself from any cause of divergence, not just
  // a known one. Runs at device init only — the live getCapabilityOptions()
  // read is precisely what sync*Units() avoids doing on every poll (see its
  // comment below), and a marker can only go stale while the app is down or
  // mid-migration anyway. An unreadable option counts as a lying marker, so a
  // throw costs one redundant write instead of a permanently wrong label.
  async reconcileUnitMarkers(device) {
    const markers = {
      appliedDistanceUnits: ['measure_range', 'measure_odo', 'meter_distance'],
      appliedSpeedUnits: ['measure_speed'],
      appliedFuelEconomyUnits: ['meter_power.fuel_economy'],
    };
    for (const [marker, caps] of Object.entries(markers)) {
      const applied = device.getStoreValue(marker);
      const present = caps.filter((cap) => device.hasCapability(cap));
      if (!applied || !present.length) continue;
      const inPlace = present.every((cap) => {
        try {
          return device.getCapabilityOptions(cap)?.units?.en === applied;
        } catch (error) {
          device.error(error);
          return false;
        }
      ...
      if (inPlace) continue;
      device.log(`${device.getName()} unit marker ${marker} claims ${applied} but the capability says otherwise, clearing it`);
      // eslint-disable-next-line no-await-in-loop
      await device.unsetStoreValue(marker).catch((error) => device.error(error));
    }
  },
