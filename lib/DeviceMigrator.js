  ...
  async syncDistanceUnits(device, imperial) {
    ...
    for (const cap of caps) {
      ...
      try {
        ...
        // hasCapability() can say true while a concurrent add/removeCapability
        // (e.g. migrateCapabilities()) still has this cap in an inconsistent
        // state on Homey's side — skip just this cap rather than aborting the
        // rest of the loop; it'll be retried next poll.
        device.error(error);
        continue;
      }
